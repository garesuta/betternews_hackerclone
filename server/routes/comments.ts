...
import type { Comment, PaginatedResponse, SuccessResponse } from "@/shared/types";
...
export const commentsRouter = new Hono<Context>()
    ...
    ).get("/:id/comments", zValidator("param", z.object({ id: z.coerce.number() }))
        ...
        async (c) => {
            ...
            const comments = await db.query.comments.findMany({
                ...
                with: {
                    ...
                    commentUpvotes: {
                        columns: { userId: true },
                        where: user?.id ? eq(commentUpvotesTable.userId, user.id) : undefined,
                        limit: 1,
                    }
                },
                extras: {
